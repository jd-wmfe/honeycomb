...
export class DatabaseClient {
	...
	async updateConfig(
		id: number,
		config: Updateable<ConfigsTable>,
	): Promise<boolean> {
		// 先检查配置是否存在
	 ...
	 * 根据配置 ID 查询所有工具
	 */
	async getToolsByConfigId(
		configId: number,
	): Promise<Selectable<ToolsTable>[]> {
		const results = await this.db
			.selectFrom(TABLES.TOOLS)
			.selectAll()
	...
	async getConfigWithTools(
		...
		(Selectable<ConfigsTable> & { tools: Selectable<ToolsTable>[] }) | null
	> {
	 ...
	 */
	async transaction<T>(
		callback: (trx: Kysely<KyselyDatabase>) => Promise<T>,
	): Promise<T> {
