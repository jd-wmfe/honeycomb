...
export function useConfigActions(
	...
	const handleStart = async (id: number) => {
		...
		consola.info(
			`[Client] 用户请求启动服务: id=${id}, name=${config?.name || "unknown"}`,
		);
	...
	const handleStop = async (id: number) => {
		const config = findConfig(id);
		consola.info(
			`[Client] 用户请求停止服务: id=${id}, name=${config?.name || "unknown"}`,
		);
	...
	const handleDelete = async (id: number) => {
		const config = findConfig(id);
		consola.info(
			`[Client] 用户请求删除服务: id=${id}, name=${config?.name || "unknown"}`,
		);
		...
		try {
			...
			const startTime = Date.now();
			consola.warn(
				`[Client] 用户确认删除服务: id=${id}, name=${config?.name || "unknown"}`,
			);
			...
			try {
				...
				if (response.code === 200) {
					consola.success(
						`[Client] 服务删除成功 (耗时: ${duration}ms): id=${id}`,
					);
	...
	const handleEdit = async (id: number) => {
		...
		const startTime = Date.now();
		consola.info(
			`[Client] 用户请求编辑配置: id=${id}, name=${config?.name || "unknown"}`,
		);
